 ...
 */
public class BuyProductDialogController implements Initializable {
    @FXML
    ...
    public void initialize(URL arg0, ResourceBundle arg1) {
        ...
        FoucsEvent(price_field);

        OnlyNumbers(quantity_field);
        OnlyNumbers(price_field);

        add_btn.setOnAction(e -> {
            error_text.setText("Button Pressed");
            int q = ToNumber(quantity_field.getText());
            float p = ToNumber(price_field.getText());

            float r1 = Selected.getQuantity() * Selected.getBuing_price();
            float r2 = p * q;
            Selected.setQuantity(Selected.getQuantity() + q);
            Selected.setBuing_price((r1 + r2) / Selected.getQuantity());
            ...
            if (PopUpWindows.ShowResult(ProductsDatabase.setProduct(Selected))) {
                ...
                Stage stage = (Stage) price_field.getScene().getWindow();
                stage.close();
            }
        });
    }
